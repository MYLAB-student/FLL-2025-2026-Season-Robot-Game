"""
【開発版プログラムセレクター】
このファイルは、selecter.pyの「開発版（デベロップメント版）」です。
通常版よりも高機能で、デバッグ（バグ探し）に便利な機能があります。

【通常版（selecter.py）との違い】
...
async def selecter_task():
    ...
    【この関数の役割】
    通常版のselecter.pyと同じ機能を、非同期処理で実現します。
...
if dev:
    ...
    # 
    # 【実行されるタスク】
    # 1. selecter_task() : ボタンでプログラムを選択・実行（通常版と同じ）
    print("--- 本番モードで起動（センサーログなし） ---")
    run_task(multitask(
        selecter_task()               # プログラム選択・実行タスク（単独）
    ))
