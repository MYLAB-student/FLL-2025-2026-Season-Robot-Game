...
├── README.md              # このファイル
├── selector.py           # プログラム選択・実行インターフェース（競技用）
├── selector_dev.py       # プログラム選択・実行インターフェース（開発用・ログ機能付き）
...
### デバッグ方法
1. センサーログを確認（`selector_dev.py`で`dev = True`）
2. 各ステップで`print()`文を追加
3. エラーメッセージを確認
4. `robot.stop()`と`robot.reset()`でリソースをクリア

### パフォーマンス最適化
- 競技時は`selector.py`（ログなし）を使用
- 不要な`wait()`を削除
- 並行処理（`wait=False`）の活用
- PID設定の最適化
